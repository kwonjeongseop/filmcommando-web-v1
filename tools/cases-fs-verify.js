...
(async () => {
  ...
  await pg.goto(
    'https://filmcommando.com/Cases.dc.html',
    {waitUntil:'domcontentloaded'});
  await pg.waitForTimeout(3000);
  await pg.screenshot({
    path:path.join(SS,ts+'_cases-fs_pass.png'),
    fullPage:false });
  const r = await pg.evaluate(()=>({
    gridExists:  !!document.getElementById('cs-grid'),
    emptyVisible:
      document.getElementById('cs-empty')
        ?.style.display !== 'none',
    statsExists: !!document.getElementById('cs-stats'),
  ...
  fs.writeFileSync(
    path.join(LOG,ts+'_cases-fs-verify.json'),
    JSON.stringify(r,null,2),'utf8');
  console.log('스크린샷:',SS);
  console.log('로그:',
    LOG+ts+'_cases-fs-verify.json');
  await br.close();
})().catch(e => {
