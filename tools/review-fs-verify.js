...
(async () => {
  ...
  await pg.goto(
    'https://filmcommando.com/Review.dc.html',
    {waitUntil:'domcontentloaded'});
  await pg.waitForTimeout(3000);
  await pg.screenshot({
    path:path.join(SS,ts+'_review-fs_pass.png'),
    fullPage:false });
  const r = await pg.evaluate(()=>({
    listExists:  !!document.getElementById('rv-list'),
    emptyVisible:
      document.getElementById('rv-empty')
        ?.style.display !== 'none',
    formExists:  !!document.getElementById('rv-form-wrap'),
    formHidden:
      document.getElementById('rv-form-wrap')
        ?.style.display === 'none',
    scForGone:   !document.querySelector(
      '[list*="reviews"]'),
  ...
  fs.writeFileSync(
    path.join(LOG,ts+'_review-fs-verify.json'),
    JSON.stringify(r,null,2),'utf8');
  console.log('스크린샷:',SS);
  console.log('로그:',
    LOG+ts+'_review-fs-verify.json');
  await br.close();
})().catch(e => {
